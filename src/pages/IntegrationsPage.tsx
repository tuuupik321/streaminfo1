...
import { Twitch, Youtube, Send, Heart, Sparkles, Flame, ArrowRight, Link2, Radio } from "lucide-react";
...
type Platform = "twitch" | "youtube" | "telegram" | "donatealerts" | "kick" | "vklive";
...
function buildChannelUrl(platform: Platform, channel: string) {
  ...
  if (platform === "vklive") return `https://vkplay.live/${trimmed.replace(/^@/, "")}`;
  return trimmed;
}
...
async function postVerifyChannel(payload: VerifyPayload) {
  ...
  if (payload.platform === "twitch" || payload.platform === "youtube") {
    ...
    return fetch("/api/channel/connect", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ user_id: payload.user_id, channel_url, init_data: payload.init_data }),
    });
  }
  return response;
}
...
const platforms: PlatformConfig[] = [
  ...
  },
  {
    ...
    label: "VK Live",
    color: "#2787F5",
    placeholder: "https://vkplay.live/username",
    description: "Связывает VK Live с аналитикой и общей картиной по площадкам.",
    category: "platforms",
    icon: Radio,
  },
  {
    ...
    description: "Позволяет отправлять анонсы и публиковать ссылки через сайт, приложение и mini app.",
...
const extractHandle = (value: string) => {
  ...
  if (raw.includes("vkplay.live/")) {
    return raw.split("vkplay.live/")[1].split(/[/?#]/)[0];
  }
  if (raw.includes("live.vkplay.ru/")) {
    return raw.split("live.vkplay.ru/")[1].split(/[/?#]/)[0];
  }
  if (raw.includes("vk.com/")) {
    return raw.split("vk.com/")[1].split(/[/?#]/)[0];
  }
  return raw;
};
...
function mapIntegrationError(
  ...
  if (errorCode === "user_mismatch") {
    return t("integrations.errorAuth", "Авторизуйтесь в приложении или на сайте для проверки.");
  }
...
export default function IntegrationsPage() {
  ...
  const platformLabels = useMemo(() => {
    return platforms.reduce<Record<Platform, string>>((acc, p) => {
      acc[p.key] = p.label;
      return acc;
    }, { twitch: "Twitch", youtube: "YouTube", telegram: "Telegram", donatealerts: "DonateAlerts", kick: "Kick", vklive: "VK Live" });
  }, []);
  const [showSuccess, setShowSuccess] = useState(false);
  const [successData, setSuccessData] = useState<VerifyResult | null>(null);
  const integrationCategoryMeta = useMemo(() => ({
    platforms: {
      title: t("integrations.groupPlatforms", "Платформы"),
      description: t("integrations.groupPlatformsDesc", "Подключите Twitch, YouTube, VK Live и Kick, чтобы открыть аналитику и историю эфиров."),
    },
    donations: {
      title: t("integrations.groupDonations", "Донаты"),
      description: t("integrations.groupDonationsDesc", "Подключите сервис поддержки, чтобы видеть историю и топ донатеров."),
    },
    notifications: {
      title: t("integrations.groupNotifications", "Уведомления"),
      description: t("integrations.groupNotificationsDesc", "Подключите Telegram, чтобы отправлять анонсы через сайт или mini app."),
    },
  }) as const, [t]);
  ...
  const verify = async () => {
    ...
    if (!activePlatform || !hasVerifiedContext) {
      setStatus("error");
      setErrorMessage(t("integrations.errorAuth", "Авторизуйтесь в приложении или на сайте для проверки."));
      return;
    }
    setStatus("verifying");
    setErrorMessage(null);
  ...
  const confirmConnection = async () => {
    ...
    if (hasVerifiedContext) {
      ...
      if (result.platform === "vklive") {
        const vkliveName = extractHandle(result.url || result.name);
        response = await fetch("/api/save_settings", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ user_id: userId, vklive_name: vkliveName, init_data: initData }),
        });
      }
  ...
  const primaryNextStep = !hasVerifiedContext
    ? t("integrations.nextStepTelegram", "Основной канал можно подключить прямо сейчас. Для Telegram и Kick приложение попросит открыть его внутри Telegram.")
    : connected.twitch || connected.youtube || connected.kick || connected.vklive
      ? t("integrations.nextStepMore", "Подключите ещё одну площадку, чтобы видеть общую аналитику по платформам.")
      : t("integrations.nextStepFirst", "Подключите платформу, чтобы открыть аналитику, историю эфиров и AI-рекомендации.");
      ...
      <motion.section variants={item} className="saas-card mb-6 overflow-hidden">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            <div className="text-[11px] font-medium uppercase tracking-[0.28em] text-white/45">{t("integrations.title", "Интеграции")}</div>
            <h1 className="mt-3 text-2xl font-black text-white md:text-3xl">{t("integrations.heroTitle", "Подключения, которые открывают весь mini app")}</h1>
            <p className="mt-3 text-sm leading-6 text-white/65">
              {t("integrations.heroDesc", "Платформы дают аналитику и историю эфиров, а Telegram и донаты добавляют поддержку и быстрые публикации.")}
            </p>
            <div className="mt-5 rounded-[24px] border border-white/10 bg-white/5 px-4 py-4 text-sm text-white/72">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-white">{t("integrations.nextStep", "Следующий шаг")}</p>
