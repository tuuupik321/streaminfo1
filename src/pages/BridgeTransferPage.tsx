...
export default function BridgeTransferPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <div className="linear-card relative overflow-hidden p-6">
        <h1 className="mb-4 text-xl font-bold text-white">Cross-Platform Bridge</h1>
        <p className="mb-6 text-sm text-white/60">Twitch to YouTube data transfer simulation</p>

        <div className="relative h-48 overflow-hidden rounded-[6px] border border-white/10 bg-black">
          <div className="absolute left-8 top-1/2 -translate-y-1/2">
            <div className="rounded-[6px] border border-[#9146ff]/40 bg-black p-3 text-[#9146ff]">
              <Twitch size={28} />
            </div>
          </div>

          <div className="absolute right-8 top-1/2 -translate-y-1/2">
            <div className={`rounded-[6px] border p-3 transition-all duration-300 ${arrived ? "border-red-500 bg-red-500/20 text-red-400" : "border-red-500/40 bg-black text-red-500"}`}>
              <Youtube size={28} />
            </div>
          </div>
          ...
          {arrived && (
            <motion.div
              ...
              className="absolute right-24 top-12 rounded-[6px] border border-white/15 bg-black/70 px-3 py-2 backdrop-blur"
            >
        ...
        <button
          type="button"
          onClick={start}
          className="mt-5 rounded-[4px] border border-white/30 px-4 py-2 text-xs font-semibold tracking-[0.12em] text-white uppercase transition-all hover:bg-white hover:text-black"
        >
          Start Transfer
        </button>
      </div>
