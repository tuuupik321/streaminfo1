      ...
      {open && (
        ...
        >
          ...
          >
            ...
            <h3 className="mt-2 text-lg font-semibold">{steps[stepIndex]?.title}</h3>
            <p className="mt-2 text-sm text-muted-foreground">{steps[stepIndex]?.body}</p>
            <div className="mt-5 flex items-center justify-between gap-3">
                ...
                {steps.map((_, index) => (
                  <span
                    ...
                    className={cn(
                      ...
                      index === stepIndex ? "bg-emerald-400/80" : "bg-white/15",
                    )}
                  />
                ))}
              </div>
              <button
                type="button"
                onClick={handleNext}
                className="inline-flex items-center gap-2 rounded-full bg-emerald-400/90 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-emerald-950 shadow-[0_12px_30px_rgba(16,185,129,0.35)] transition hover:bg-emerald-300 active:scale-[0.98]"
              >
                {t("onboarding.next", "Далее")}
              </button>
            </div>
            <div className="mt-3 text-[10px] uppercase tracking-[0.26em] text-muted-foreground">
