...
export function BottomNav({ onOpenSettings }: { onOpenSettings?: (anchor: DOMRect | null) => void }) {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 md:hidden">
      <div className="mx-auto max-w-[24rem] px-3 pb-[calc(env(safe-area-inset-bottom)+0.55rem)]">
        <div className="relative grid grid-cols-5 gap-1 rounded-[1.8rem] border border-white/10 bg-[hsl(var(--card))/0.97] p-1.5 shadow-[0_24px_56px_rgba(3,12,24,0.46)] backdrop-blur-[24px] before:pointer-events-none before:absolute before:inset-x-6 before:top-0 before:h-px before:bg-white/12">
          {mainTabs.map((tab) => {
            ...
            return (
              <NavLink
                key={tab.path}
                to={tab.path}
                className="group relative flex h-[3.2rem] flex-col items-center justify-center gap-1 overflow-hidden rounded-[1.2rem]"
              >
                {isActive ? (
                  <motion.span
          ...
          <button
            ...
            className="group relative flex h-[3.2rem] flex-col items-center justify-center gap-1 overflow-hidden rounded-[1.2rem] text-muted-foreground/90 transition-all duration-200 active:scale-95"
